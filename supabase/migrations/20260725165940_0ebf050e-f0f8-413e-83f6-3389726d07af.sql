-- Preview: nada é alterado
CREATE OR REPLACE FUNCTION public.platform_reset_preview()
...
-- Execução: transação única, aborta tudo em caso de erro
CREATE OR REPLACE FUNCTION public.platform_reset_execute()
...
BEGIN
  ...
  VALUES (v_actor, 'platform_environment_reset', 'platform', 'reset', v_report, NULL);

  RETURN v_report;
END $$;

REVOKE EXECUTE ON FUNCTION public.platform_reset_preview() FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.platform_reset_execute() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.platform_reset_preview() TO service_role;
GRANT EXECUTE ON FUNCTION public.platform_reset_execute() TO service_role;