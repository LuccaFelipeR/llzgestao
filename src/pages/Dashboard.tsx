...
import { Package, MapPin, Boxes, AlertTriangle, ArrowRightLeft, TrendingUp, ShoppingCart, Download, BarChart3, ClipboardList } from "lucide-react";
...
export default function Dashboard() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { products: products.count ?? 0, addresses: addresses.count ?? 0, totalItems, staleCount, recentMovements: recentMovements.data ?? [] };
    },
  });
  ...
  const { data: criticalStock } = useQuery({
    ...
    queryFn: async () => {
      const { data: products } = await supabase.from("products").select("id, sku, description, min_stock").eq("is_active", true).gt("min_stock", 0);
      if (!products?.length) return [];
      const { data: balances } = await supabase.from("stock_balance").select("product_id, qty").gt("qty", 0);
  ...
  const { data: abcData } = useQuery({
    queryKey: ["abc-curve"],
    queryFn: async () => {
      const { data: movements } = await supabase.from("movements").select("product_id, qty").eq("type", "OUT");
      ...
      const { data: products } = await supabase.from("products").select("id, sku, description, price").eq("is_active", true);
